...
2. Kolekcja Set używa operatora  ` === ` by porównać nową wartość z obiektami mającymi identyczny kod mieszający.
...
3. Kolekcja `Set` używa operatora ` == ` by porównać nową wartość ze wszystkimi pozostałymi obiektami mającymi ten sam kod mieszający:
